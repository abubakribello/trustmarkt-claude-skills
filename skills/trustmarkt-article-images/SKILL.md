...
- Lighting/background: **mostly near-black/dark-navy (`#000046` or darker), not the full brand gradient** — a full-canvas navy-to-cyan wash was tried and came back as feedback ("too blue-ish"), same complaint the reference examples avoid: they're dark backgrounds with cyan used only as a rim-light glow on the subject's hair/shoulder and in the diagram/text accents, never filling the frame. Keep it that way — dark base, cyan as a small-area accent only.
...
- Background is mostly near-black/dark navy (`#000046` or darker) with `#1cb5e0` used only as a small-area accent (rim-light, diagram lines, text) — never a full-canvas blue wash, that read as "too blue" in feedback on the first batch.
